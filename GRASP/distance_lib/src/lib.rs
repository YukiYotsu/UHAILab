...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn unrestricted_damerau_levenshtein(s1: *const c_char, s2: *const c_char) -> usize {
    let s1 = CStr::from_ptr(s1).to_string_lossy().into_owned();
    let s2 = CStr::from_ptr(s2).to_string_lossy().into_owned();
